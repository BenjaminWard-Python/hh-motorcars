...
const globalStyles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card-hover:hover { transform: translateY(-4px); box-shadow: 0 16px 48px rgba(0,0,0,0.5); border-color: ${C.accent}55 !important; }

  .btn-primary {
    background: ${C.accent}; color: #0f0e0d; border: none; padding: 10px 22px;
    font-family: 'Barlow Condensed', sans-serif; font-size: 13px; font-weight: 600;
    letter-spacing: 1.5px; text-transform: uppercase; cursor: pointer;
    transition: background 0.2s ease, transform 0.15s ease; border-radius: 2px;
  }
  .btn-primary:hover { background: ${C.accentDark}; transform: translateY(-1px); }
  .btn-primary:active { transform: translateY(0); }

  .btn-ghost {
    background: transparent; color: ${C.text}; border: 1px solid ${C.cardBorder}; padding: 9px 20px;
  ...
  .btn-danger {
    background: transparent; color: ${C.red}; border: 1px solid ${C.red}55; padding: 7px 16px;
  ...
  input, textarea, select {
    background: #111010; border: 1px solid ${C.cardBorder}; color: ${C.text};
    font-family: 'Barlow', sans-serif; font-size: 14px; padding: 10px 14px;
    border-radius: 2px; width: 100%; outline: none; transition: border-color 0.2s;
  }
  ...
  label {
    ...
    text-transform: uppercase; color: ${C.muted}; display: block; margin-bottom: 6px;
  }
...
function Nav({ view, setView, isAdmin, setIsAdmin }) {
  return (
    <nav style={{
      ...
      borderBottom: `1px solid ${C.cardBorder}`, padding: "0 32px",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{
          ...
          fontSize: 12, fontFamily: "'Barlow Condensed'", fontWeight: 700, letterSpacing: 1, color: C.accent,
        }}>HH</div>
        <div>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        {isAdmin ? (
          <>
            ...
            <button className="btn-danger" onClick={() => { setIsAdmin(false); setView("public"); }}>Log Out</button>
          </>
        ) : (
          <button className="btn-ghost" onClick={() => setView("login")}>Admin</button>
        )}
      </div>
    </nav>
  );
    ...
    <div className="card-hover" onClick={onClick} style={{
      ...
      <div style={{ height: 200, background: "#0d0c0b", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          fontFamily: "'Barlow Condensed'", fontSize: 16, fontWeight: 600, color: C.accent, letterSpacing: 0.5,
        }}>{formatPrice(car.price)}</div>
      </div>
      <div style={{ padding: "18px 20px 20px" }}>
        <div style={{ fontFamily: "'Barlow Condensed'", fontSize: 12, color: C.muted, letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 }}>{car.year}</div>
        <div style={{ fontFamily: "'Playfair Display'", fontSize: 18, fontWeight: 600, marginBottom: 4 }}>{car.make} {car.model}</div>
        {car.trim && <div style={{ color: C.muted, fontSize: 13, marginBottom: 14 }}>{car.trim}</div>}
        ...
        <div style={{ marginTop: 16, paddingTop: 16, borderTop: `1px solid ${C.cardBorder}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ color: C.muted, fontSize: 12 }}>View Details →</span>
          <span style={{ fontFamily: "'Barlow Condensed'", fontSize: 10, letterSpacing: 1.5, textTransform: "uppercase", color: C.green, background: `${C.green}15`, padding: "2px 8px", borderRadius: 2 }}>Available</span>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ padding: "40px", maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 40 }}>
            ...
            {hasImages && car.images.length > 1 && (
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {car.images.map((img, i) => (
                  <div key={i} onClick={() => setImgIdx(i)} style={{ width: 72, height: 48, borderRadius: 2, overflow: "hidden", cursor: "pointer", border: `2px solid ${i === imgIdx ? C.accent : "transparent"}` }}>
          ...
          <div>
            ...
            <h2 style={{ fontFamily: "'Playfair Display'", fontSize: 28, fontWeight: 700, lineHeight: 1.1, marginBottom: 4 }}>{car.make} {car.model}</h2>
            {car.trim && <div style={{ color: C.muted, fontSize: 15, marginBottom: 20 }}>{car.trim}</div>}
            <div style={{ fontSize: 30, fontFamily: "'Playfair Display'", fontWeight: 600, color: C.accent, marginBottom: 28 }}>{formatPrice(car.price)}</div>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.cardBorder}`, borderRadius: 4, padding: 20 }}>
              ...
              <div style={{ fontSize: 13, color: C.muted, marginBottom: 16, lineHeight: 1.6 }}>Contact HH Motorcars to schedule a viewing or request additional details.</div>
              ...
              <a href={`mailto:sales@hhmotorcars.com?subject=Inquiry: ${car.year} ${car.make} ${car.model}`} style={{ display: "block", textDecoration: "none" }}>
                <button className="btn-primary" style={{ width: "100%", marginTop: 12 }}>Send Inquiry</button>
              </a>
            </div>
...
function Login({ onSuccess }) {
  ...
  return (
    <div style={{ minHeight: "80vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: C.card, border: `1px solid ${C.cardBorder}`, borderRadius: 4, padding: 40, width: "100%", maxWidth: 380, animation: "slideDown 0.3s ease" }}>
        ...
        {error && <div style={{ color: C.red, fontSize: 13, marginBottom: 16, padding: "8px 12px", background: `${C.red}15`, borderRadius: 2 }}>{error}</div>}
  ...
  return (
    <div style={{ padding: "40px", minHeight: "100vh" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 32 }}>
        <div>
          <div style={{ fontFamily: "'Barlow Condensed'", fontSize: 11, letterSpacing: 3, color: C.accent, textTransform: "uppercase", marginBottom: 6 }}>Admin Panel</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {inventory.map(car => (
            <div key={car.id} style={{
              background: C.card, border: `1px solid ${C.cardBorder}`, borderRadius: 4, padding: "16px 20px",
              display: "flex", alignItems: "center", gap: 20, opacity: car.status === "sold" ? 0.6 : 1, animation: "fadeIn 0.3s ease",
            }}>
              <div style={{ width: 90, height: 60, flexShrink: 0, borderRadius: 2, overflow: "hidden", background: "#0d0c0b" }}>
                {car.images && car.images[0] ? <img src={car.images[0]} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <CarPlaceholder color={C.cardBorder} />}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 2 }}>
                  <span style={{ fontFamily: "'Playfair Display'", fontSize: 16, fontWeight: 600 }}>{car.year} {car.make} {car.model}</span>
                  {car.trim && <span style={{ fontSize: 12, color: C.muted }}>{car.trim}</span>}
                  <span className="tag" style={{ background: car.status === "active" ? `${C.green}20` : `${C.muted}20`, color: car.status === "active" ? C.green : C.muted }}>{car.status}</span>
                </div>
                <div style={{ color: C.muted, fontSize: 13 }}>{formatPrice(car.price)} {car.mileage && `· ${car.mileage} mi`} {car.color && `· ${car.color}`}</div>
              </div>
              <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                <button className="btn-ghost" style={{ fontSize: 12, padding: "6px 14px" }} onClick={() => toggleStatus(car.id)}>{car.status === "active" ? "Mark Sold" : "Relist"}</button>
                <button className="btn-ghost" style={{ fontSize: 12, padding: "6px 14px" }} onClick={() => setEditing(car)}>Edit</button>
    ...
    <div style={{ padding: "40px", minHeight: "100vh", animation: "fadeIn 0.3s ease" }}>
      <div style={{ maxWidth: 700, margin: "0 auto" }}>
        ...
        <div className="field-group">
          ...
          <textarea value={form.description} onChange={e => set("description", e.target.value)} rows={5} placeholder="Describe this vehicle — condition, notable features, recent work, history..." style={{ resize: "vertical" }} />
        </div>
        <div className="field-group">
          <label>Photos</label>
          <div style={{ background: "#111010", border: `1px solid ${C.cardBorder}`, borderRadius: 2, padding: 16, marginBottom: 10 }}>
            ...
            <button className="btn-ghost" onClick={() => fileInputRef.current.click()} style={{ width: "100%", marginBottom: 12 }}>📁 Upload Photos from Device</button>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <input value={imageInput} onChange={e => setImageInput(e.target.value)} placeholder="Or paste an image URL and click Add" onKeyDown={e => e.key === "Enter" && addImage()} style={{ flex: 1 }} />
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {form.images.map((img, i) => (
                <div key={i} style={{ position: "relative", width: 100, height: 70 }}>
                  ...
                  <button onClick={() => removeImage(i)} style={{ position: "absolute", top: 2, right: 2, background: "rgba(0,0,0,0.8)", border: "none", color: "#fff", width: 20, height: 20, borderRadius: "50%", cursor: "pointer", fontSize: 12, lineHeight: 1 }}>✕</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
          <button className="btn-primary" onClick={() => onSave(form)} disabled={!valid} style={{ opacity: valid ? 1 : 0.4, cursor: valid ? "pointer" : "not-allowed" }}>Save Vehicle</button>
...
export default function App() {
  ...
  const [inventory, setInventory] = useState(() => {
    ...
    try {
      const saved = localStorage.getItem("hh-inventory");
  ...
  useEffect(() => {
    try {
      localStorage.setItem("hh-inventory", JSON.stringify(inventory));
    } catch (e) {
